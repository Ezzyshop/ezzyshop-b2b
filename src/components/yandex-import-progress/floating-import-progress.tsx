import { type YandexImportJob } from "@/contexts/yandex-import-context/yandex-context-provider";
...
const percent = (job: YandexImportJob) => {
  if (job.status === "done") return 100;
  if (!job.total) return 0;
  return Math.round((job.current / job.total) * 100);
};

const StatusIcon = ({ job }: { job: YandexImportJob }) => {
  if (job.status === "done")
...
const JobCard = ({ job }: { job: YandexImportJob }) => {
  const { dismissJob } = useYandexImport();
...
export const YandexFloatingImportProgress = () => {
  const { jobs } = useYandexImport();

  if (!jobs.length) return null;

  return (
    <div className={cn("fixed bottom-4 right-4 z-50 flex flex-col gap-2 items-end")}>
      {jobs.map((job) => (
